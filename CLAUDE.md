...
1. **Retry counters never reset to 0.** Reconcilers that heal stuck jobs must *increment* `attempt_count`, cap at the column's `max_attempts`, and mark `dead` above the cap. The column defaults live in `internal/db/migrate.go` and match prod: `serp_jobs.max_attempts` defaults to **3** (`INTEGER NOT NULL DEFAULT 3`), `enrichment_jobs.max_attempts` defaults to **5** (`INTEGER DEFAULT 5`). Earlier wording here claimed 15 for enrich and 10 for serp — that was wrong and let a retry cap be reasoned about against numbers the schema never had. Grep for `attempt_count = 0` after touching any reconciler — the same antipattern keeps creeping back across `serp.go` and `enrich.go`.
...
10. **Trigger function bodies are versioned — never let a binary silently downgrade one.** `trg_normalize_enrichment` and `trg_enqueue_enrichment` (`internal/db/migrate.go`) each carry a version const (`normalizeTriggerVersion`, `enqueueTriggerVersion`, format `YYYYMMDD`) embedded in the function body as a `-- <fn> version: NNNNNNNN` marker. `replaceTriggerFunctionIfNewer` reads the live marker via `liveTriggerFnVersion` before every `CREATE OR REPLACE FUNCTION` and skips the replace (with a `slog.Warn`) if the live version is newer than the binary's — this is what stops a stale binary from downgrading a trigger a newer deploy already upgraded (2026-09-25 incident: remote workers stuck on `v0.8.3-niche` called `db.Migrate()` on every boot and silently reverted `trg_normalize_enrichment` to its old definition, so the v0.9.0–v0.9.8 classifier fixes never took effect in prod). **Bump the relevant const whenever you change that trigger function's body.** The guard only protects binaries that contain it — a stale binary still needs a real deploy to pick up newer trigger logic; it just can no longer regress the live one.
