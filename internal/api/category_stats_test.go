...
func TestIsMatviewNotPopulated(t *testing.T) {
	...
	concurrentErr := &pq.Error{Code: "55000", Message: "CONCURRENTLY cannot be used when the materialized view is not populated"}
	if !isMatviewNotPopulated(concurrentErr) {
		t.Error("pq 55000 CONCURRENTLY-not-populated (REFRESH) should classify as not-populated")
	}
